...
user_data = <<-EOF
               ...
               sudo git clone  https://github.com/namrata4447/upload-images.git /var/www/html
            EOF
}
...
#Download the image from github
   provisioner "local-exec" {
     command = "git clone https://github.com/namrata4447/upload-images.git namrata-tf-image"
 }
}
resource "aws_s3_object" "image_upload" {
	bucket = aws_s3_bucket.namrata-ebs2-bucket.bucket
        key = "Taj-Mahal.jpg"    
	source = "namrata-tf-image/Taj-Mahal.jpg"
        acl = "public-read"
}
